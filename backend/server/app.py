...
def _load_env_file() -> None:
    """Load the project-root .env into os.environ before any LLM config is read.

    The Config layer reads providers/keys (FAST_LLM, SMART_LLM, EMBEDDING,
    DASHSCOPE_API_KEY, ...) straight from os.environ. Nothing else loads the
    .env, so without this the config silently falls back to the OpenAI
    defaults and fails with "OPENAI_API_KEY must be set". Self-contained so it
    works without python-dotenv. Existing env vars take precedence.
    """
    ...
    for line in env_path.read_text(encoding="utf-8").splitlines():
        ...
        if key and key not in os.environ:
            os.environ[key] = value.split("#", 1)[0].strip().strip("\"'")
...
async def write_report(research_request: ResearchRequest, research_id: str = None):
    report_information = await run_agent(
        ...
        academic_sources=research_request.academic_sources,
        year_from=research_request.year_from,
        year_to=research_request.year_to,
        max_papers=research_request.max_papers,
        language=research_request.language,
        enable_citation_audit=research_request.enable_citation_audit,
    )
    ...
    if research_request.report_type != "multi_agents":
        ...
        academic_result = getattr(researcher, "academic_result", None)
        if academic_result:
            response["academic_result"] = {
                "papers": academic_result.get("papers", []),
                "paper_summaries": academic_result.get("paper_summaries", []),
                "taxonomy": academic_result.get("taxonomy", []),
                "citation_audit": academic_result.get("citation_audit", {}),
                "bibtex": academic_result.get("bibtex", ""),
            }
    else:
