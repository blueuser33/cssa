const GPTResearcher = (() => {
  ...
  const init = () => {
    ...
    // Initialize academic survey options
    initAcademicSurveyOptions();
  ...
  const initAcademicSurveyOptions = () => {
    const reportTypeSelect = document.querySelector('select[name="report_type"]');
    const academicOptions = document.getElementById('academicOptions');
    if (!reportTypeSelect || !academicOptions) return;

    const updateAcademicOptions = () => {
      academicOptions.style.display = reportTypeSelect.value === 'academic_survey' ? 'block' : 'none';
    };

    reportTypeSelect.addEventListener('change', updateAcademicOptions);
    updateAcademicOptions();
  }
  ...
  const listenToSockEvents = () => {
    ...
    socket.onopen = (event) => {
      ...
      if (report_type === 'academic_survey') {
        Object.assign(requestData, {
          academic_sources: ['arxiv'],
          year_from: parseInt(document.getElementById('yearFrom')?.value, 10) || null,
          year_to: parseInt(document.getElementById('yearTo')?.value, 10) || null,
          max_papers: parseInt(document.getElementById('maxPapers')?.value, 10) || 30,
          language: document.getElementById('academicLanguage')?.value || 'zh',
          enable_citation_audit: document.getElementById('enableCitationAudit')?.checked ?? true,
        });
      }
