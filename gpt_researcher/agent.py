...
from .skills.academic_researcher import AcademicResearcher
...
class GPTResearcher:
    ...
    async def conduct_research(self, on_progress=None):
        ...
        if self.report_type == ReportType.AcademicSurvey.value:
            self._current_step = "academic_survey"
            return await self.conduct_academic_survey()
    ...
    async def conduct_academic_survey(
        self,
        ...
        result = await AcademicResearcher(
            self,
            academic_sources=academic_sources,
            year_from=year_from,
            year_to=year_to,
            max_papers=max_papers,
            language=language or getattr(self.cfg, "language", "zh"),
            style=style,
            enable_citation_audit=enable_citation_audit,
        ).run()
        self.context = result
        await self._log_event("research", step="academic_survey_completed", details={
            "paper_count": len(result.get("papers", [])),
            "citation_audit_checked": result.get("citation_audit", {}).get("checked", 0)
            if isinstance(result.get("citation_audit"), dict)
            else 0,
        })
        return result
